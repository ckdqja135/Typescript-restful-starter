## Q. services에 있는 코드와 repository에 있는 코드의 차이점에 대해.
...
* A. 
```text
제가 이해하고 있는 바는 service layer 에는 비지니스 로직이 들어가고 repository layer 에는 ORM을 활용한 Query문이 들어간다는 것입니다.
예를 들어 영화 예매시스템을 만든다고 했을때 단순히 한 repository 에 해당 Query가 실행 되는 것이 아니라 잔액 확인, 포인트 차감, 좌석 확인, 회원 확인등 다수의 repository + 다양한 비지니스 로직이 필요합니다.
이럴 경우 service layer = repository layer 일 경우는 비지니스 로직과 ORM 로직이 섞이고 다른 비지니스 로직에서 ORM 을 재사용하기 어렵기 때문에
이러한 비지니스 로직은 service layer에 작성하고 세세한 ORM query는 repository layer 에 추가 되는 걸로 알고 있습니다.
```
